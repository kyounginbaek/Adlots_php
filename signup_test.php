			...
			if(mysqli_num_rows($query) > 0) {
				...
				if(mysqli_num_rows($query_email) > 0) {
					...
					if(empty($recommend)){ //추천인이 비어있을 경우, 기존과 똑같이 진행
						mysqli_query($this-> connection, "INSERT INTO adlots_users (`email`,`password`,`phone`,`nickname`,`when`,`latest_signin`) VALUES ('$email','$password','$phone','$nickname','$latest_signin','$latest_signin') ");
